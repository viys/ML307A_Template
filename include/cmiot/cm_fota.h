...
typedef enum
{
    CM_FOTA_ASR_PLAN_INVALID = 0,
    CM_FOTA_ASR_PLAN_MINI_INTEGRATE,  /** ASR 整包 mini FOTA方案 */
    CM_FOTA_ASR_PLAN_MINI,            /** ASR 2包 mini FOTA方案 */
...
typedef enum
{
    ...
    CM_FOTA_TYPE_FTP,
    CM_FOTA_TYPE_HTTP_HTTPS            /*!< 仅支持HTTP下载方式，不支持HTTPS下载方式 */
} cm_fota_url_type_e;
...
typedef struct
{
    ...
    cm_fota_url_type_e fota_mode;        /*!< fota升级模式, 0: HTTP/HTTPS */
 ...
 *           ML307A-DCLN ML305A-DC支持HTTP下载方式，ML307A-DSLN ML307A-GSLN ML305A-DS支持HTTP、FTP下载方式，暂不支持HTTPS、FTPS下载方式 \n
