...
// int sband = 0;
    int sinr = 0;
    uint32_t rat = CM_STATUS_RAT_INVALID;
    char mnc[8] = {0};
    char mcc[8] = {0};
    char at_resp[256] = {0};
    ...
    if(0 == osMessageQueueGet(modem_mq, modem_flag, 0, CB_MSG_OUTTIME))
    {
        ...
        {   
            cm_virt_at_get(at_data, sizeof(at_data)); 
            strcpy(data, (char *)at_data); 
            if(NULL == strstr(data, "+ZDON") || NULL == strstr(data, "+EEMLTESVC"))
            {
                return -1;
            }
            /** Parse mcc and mnc */
            strcpy(at_resp, strstr(data, "+ZDON"));
            if (0 == strncmp(at_resp, "+ZDON", 5))
            {
                result = strtok(at_resp, ":");
                if (NULL != result)
                    result = strtok(NULL, ":");
                result = strtok(result, spl);
                while (NULL != result)
                {
                    item++;
                    if (2 == item)
                    {
                        ...
                        {
                            result++;
                            strncpy(mcc, result, strlen(result) - 1);
                        }
                    }
                    else if(3 == item)
                    {
                        ...
                        {
                            result++;
                            strncpy(mnc, result, strlen(result) - 1);
                        }
                        break;
                    }
        ...
        if(strstr(data, "+COPS"))
        {
            memset(at_resp, 0, 256);
            strcpy(at_resp, strstr(data, "+COPS"));
            if (0 == strncmp(at_resp, "+COPS", strlen("+COPS")))
            {
                item = 0;
                result = strtok(at_resp, ":");
                if (NULL != result)
                ...
                while (NULL != result)
                {
                    ...
                    {
                        if(7 == atoi(result))
                            rat = CM_STATUS_RAT_LTE;    /** LTE only */
                    }
                    result = strtok(NULL, spl);
                }
            }
        ...
        {
            memset(at_resp, 0, 256);
            strcpy(at_resp, strstr(data, "+CESQ"));
            if (0 == strncmp(at_resp, "+CESQ", strlen("+CESQ")))
            {
                item = 0;
                result = strtok(at_resp, ":");
                if (NULL != result)
                ...
                while (NULL != result)
                {
                    ...
                    if (5 == item)
                    {
                        ...
                        {
                            rsrq = ((-195 + 5 * rsrq_cesq) > -30) ? -30 : (-195 + 5 * rsrq_cesq);
                        }
                    }
                    result = strtok(NULL, spl);
                }
        ...
        /** Parse others */
        memset(at_resp, 0, 256);
        strcpy(at_resp, strstr(data, "+EEMLTESVC"));
        if (0 == strncmp(at_resp, "+EEMLTESVC", strlen("+EEMLTESVC")))
        {
            item = 0;
            result = strtok(at_resp, ":");
            if (NULL != result)
            ...
            radio_info->rat = rat;
            radio_info->rsrp = rsrp;
            radio_info->rssi = rssi;
            radio_info->tx_power = -32768;                            /** 无效值 */
            radio_info->tx_time = 0;           
            radio_info->rx_time = 0;           
            radio_info->last_cellid = 0xFFFFFFFF;                    /** 无效值 */
            radio_info->last_ecl = 255;
            radio_info->last_snr = sinr;
            radio_info->last_earfcn = -1;
            radio_info->last_pci = 65535;
            radio_info->rsrq = rsrq;
            radio_info->rxlev = 0;
        }                                
    } 
    }
...
//  int sband = 0;
    ...
    if(0 == osMessageQueueGet(modem_mq, modem_flag, 0, CB_MSG_OUTTIME))
    {
        ...
        if(strstr(data, "+COPS"))
        {
            memset(at_resp, 0, 256);
            strcpy(at_resp, strstr(data, "+COPS"));
            if (strncmp(at_resp, "+COPS", strlen("+COPS")) == 0)
            {
                item = 0;
                result = strtok(at_resp, ":");
                if (NULL != result)
       ...
       if(strstr(data, "+CESQ"))
        ...
        if (0 == strncmp(at_resp, "+EEMLTESVC", strlen("+EEMLTESVC")))
        {
            ...
            cell_info[0].primary_cell = rat;
            memcpy(cell_info[0].mcc, mcc, sizeof(mcc));
            memcpy(cell_info[0].mnc, mnc, sizeof(mnc));
            cell_info[0].earfcn = earfcn;
            cell_info[0].pci = pci;
            cell_info[0].rsrp = rsrp;
            cell_info[0].rsrq = rsrq; 
            cell_info[0].rssi = rssi;
            cell_info[0].snr = sinr;
            cell_info[0].earfcn_offset = ' ';
            cell_info[0].bandwidth = ' ';
            get_cell_info = 1;        //成功，只支持1组小区信息                          
        }  
    }
    osMessageQueueDelete(modem_mq);         
    cm_virt_at_deinit();  
...
int32_t cm_modem_get_cereg_state(cm_cereg_state_t *cereg)
{
    ...
    modem_mq = osMessageQueueNew(CB_MSG_COUNT, CB_MSG_SIZE, NULL);    
    if(cm_virt_at_send((uint8_t *)"AT+CEREG=5\r\n", strlen("AT+CEREG=5\r\n")) < 0)
    {
        return -1;
    }
    if(0 == osMessageQueueGet(modem_mq, modem_flag, 0, CB_MSG_OUTTIME))
